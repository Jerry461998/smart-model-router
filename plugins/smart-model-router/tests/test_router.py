...
class RequiredRoutingTests(unittest.TestCase):
    ...
    def test_02_django_crud_uses_luna_sol_luna(self):
        ...
        self.assertEqual(models(result), ["gpt-6-luna", "gpt-6-sol", "gpt-6-luna"])
        self.assertEqual(result["default_root"], {"model": "gpt-6-sol", "effort": "medium"})
    ...
    def test_04_production_502_not_luna_only(self):
        ...
        self.assertEqual(models(result)[:2], ["gpt-6-luna", "gpt-6-sol"])
        self.assertIn("gpt-6-sol", models(result))
    ...
    def test_05_cicd_architecture_route(self):
        ...
        self.assertEqual(
            models(result),
            ["gpt-6-luna", "gpt-6-sol", "gpt-6-sol", "gpt-6-luna", "gpt-6-sol"],
        )
    ...
    def test_08_small_complex_race_uses_sol_reasoning(self):
        ...
        self.assertIn("gpt-6-sol", models(result))
        sol_phase = next(item for item in result["phases"] if item["model"] == "gpt-6-sol")
    ...
    def test_10_login_record_page_implemented_by_sol(self):
        ...
        self.assertEqual(implementation["model"], "gpt-6-sol")
...
class PolicyInvariantTests(unittest.TestCase):
    ...
    def test_17_every_selected_model_is_gpt6(self):
        ...
        for prompt in prompts:
            result = route(prompt)
            self.assertTrue(result["default_root"]["model"].startswith("gpt-6-"))
            self.assertTrue(all(model.startswith("gpt-6-") for model in models(result)))
